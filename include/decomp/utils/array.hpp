...
namespace decomp {
    ...
    typename ArrayView<T>::iterator ArrayView<T>::begin() {
        if (m_offset > m_target->m_size) {
            return m_target->m_data + m_target->m_size;
        }
    ...
    u32 ArrayView<T>::size() const {
        ...
        if ((m_offset + m_size) >= m_target->m_size) {
            return m_target->m_size - m_offset;
        }
    ...
    template <typename T>
    template <typename F>
    std::enable_if_t<std::is_invocable_r_v<bool, F, T>, const typename ArrayView<T>::PointerTp> ArrayView<T>::find(
        F&& cb
    ) const {
        for (u32 i = m_offset; i < (m_offset + m_size) && i < m_target->m_size; i++) {
            if (cb(m_target->m_data[i])) {
                if constexpr (std::is_pointer_v<T>) {
                    return m_target->m_data[i];
                } else {
                    return &m_target->m_data[i];
                }
            }
    ...
    template <typename T>
    template <typename F>
    std::enable_if_t<std::is_invocable_r_v<bool, F, T, u32>, const typename ArrayView<T>::PointerTp> ArrayView<T>::find(
        F&& cb
    ) const {
        for (u32 i = m_offset; i < (m_offset + m_size) && i < m_target->m_size; i++) {
            if (cb(m_target->m_data[i], i - m_offset)) {
    ...
    std::enable_if_t<std::is_invocable_r_v<M, F, T, u32>, Array<M>> ArrayView<T>::map(F&& cb) const {
        Array<M> out(m_size);
        for (u32 i = m_offset; i < (m_offset + m_size) && i < m_target->m_size; i++) {
            out.push(cb(m_target->m_data[i], i - m_offset));
        }
        return out;
    }
    ...
    void Array<T>::insert(u32 at, const Array<T>& v) {
        ...
        if (m_size + v.m_size >= m_capacity) {
            expand(v.m_size);
        }

        for (u32 i = m_size + v.m_size - 1; i >= (at + v.m_size); i--) {
            if (i >= m_size) {
                new (&m_data[i]) T(m_data[i - v.m_size]);
            } else {
                m_data[i] = m_data[i - v.m_size];
            }
        }

        for (u32 i = 0; i < v.m_size; i++) {
            m_data[at + i] = v.m_data[i];
        }
    ...
    Array<T> Array<T>::extract(u32 from, u32 count, bool doRemove) {
        ...
        for (u32 i = 0; i < count; i++) {
            ...
            if (doRemove && (from + i + count) < m_size) {
                m_data[from + i] = m_data[from + i + count];
            }
        }
    ...
    Array<T> Array<T>::concat(const Array<T>& arr) const {
        if ((m_size + arr.m_size) == 0) {
            return Array<T>();
        }

        Array<T> out(m_size + arr.m_size);
        for (u32 i = 0; i < m_size; i++) {
            new (&out.m_data[i]) T(m_data[i]);
        }
        for (u32 i = 0; i < arr.m_size; i++) {
            new (&out.m_data[i + m_size]) T(arr.m_data[i]);
        }
    ...
    Array<T>& Array<T>::operator=(const Array<T>& rhs) {
        ...
        for (u32 i = 0; i < rhs.m_size; i++) {
            new (&m_data[m_size++]) T(rhs.m_data[i]);
        }
        return *this;
    }
    ...
    template <typename T>
    template <typename F>
    std::enable_if_t<std::is_invocable_r_v<bool, F, T>, const typename Array<T>::PointerTp> Array<T>::find(F&& cb
    ) const {
        for (u32 i = 0; i < m_size; i++) {
            if (cb(m_data[i])) {
                if constexpr (std::is_pointer_v<T>) {
    ...
    template <typename T>
    template <typename F>
    std::enable_if_t<std::is_invocable_r_v<bool, F, T, u32>, const typename Array<T>::PointerTp> Array<T>::find(F&& cb
    ) const {
        for (u32 i = 0; i < m_size; i++) {
            if (cb(m_data[i], i)) {
                if constexpr (std::is_pointer_v<T>) {
    ...
    void Array<T>::expand(u32 minDelta) {
        if (!m_data) {
            ...
            while ((m_capacity - prevCap) < minDelta) {
                m_capacity *= 2;
            }
            m_data = (T*)new u8[sizeof(T) * m_capacity];
            return;
        }
        ...
        T* newData = (T*)new u8[sizeof(T) * m_capacity];
