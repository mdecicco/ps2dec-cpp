...
namespace render {
    namespace vulkan {
        ...
        GraphicsPipeline::GraphicsPipeline(
            ShaderCompiler* compiler, LogicalDevice* device, SwapChain* swapChain, RenderPass* render
        )
            : Pipeline(device), decomp::IWithLogging("Vulkan Pipeline") {
            m_compiler            = compiler;
            m_device              = device;
        ...
        void GraphicsPipeline::reset() {
            ...
            m_primTypeDynamic               = false;
            m_polyMode                      = PM_FILLED;
            m_polyModeDynamic               = false;
            m_cullMode                      = CM_BACK_FACE;
            m_cullModeDynamic               = false;
            m_frontFaceMode                 = FFM_CLOCKWISE;
            m_frontFaceModeDynamic          = false;
            m_lineWidth                     = 1.0f;
            m_lineWidthDynamic              = false;
            m_primRestartEnabled            = false;
            m_primRestartEnabledDynamic     = false;
            m_depthClampEnabled             = false;
            m_depthClampEnabledDynamic      = false;
            m_depthCompareOp                = CO_GREATER;
            m_depthCompareOpDynamic         = false;
            ...
            m_alphaBlendOp                  = BO_ADD;
            m_blendEquationDynamic          = false;
            m_minDepthBounds                = 0.0f;
            m_maxDepthBounds                = 1.0f;
            m_depthBoundsDynamic            = false;
            m_depthTestEnabled              = false;
            m_depthTestEnabledDynamic       = false;
            m_depthBoundsTestEnabled        = false;
            m_depthBoundsTestEnabledDynamic = false;
        ...
        void GraphicsPipeline::addSampler(u32 bindIndex, VkShaderStageFlagBits stages) {
            m_samplers.push({bindIndex, stages});
        }

        void GraphicsPipeline::addUniformBlock(
            u32 bindIndex, const core::DataFormat* fmt, VkShaderStageFlagBits stages
        ) {
            m_uniformBlocks.push({bindIndex, stages, fmt});
        }

        void GraphicsPipeline::addStorageBuffer(u32 bindIndex, VkShaderStageFlagBits stages) {
            m_storageBuffers.push({bindIndex, stages});
        }

        void GraphicsPipeline::setPushConstantFormat(const core::DataFormat* fmt) {
            if (m_isInitialized) {
                error("Push constant format can't be changed after initialization");
                return;
            }

            u32 maxSize = m_device->getPhysicalDevice()->getProperties().limits.maxPushConstantsSize;
            if (fmt->getUniformBlockSize() > maxSize) {
                error(
                    "Push constant format size (size: %d bytes, aligned size: %d bytes) exceeds max push constant size "
                    "of %d bytes",
                    fmt->getSize(),
                    fmt->getUniformBlockSize(),
                    maxSize
                );
                return;
            }

            m_pushConstantFormat = fmt;
        }
        ...
        void GraphicsPipeline::setViewport(f32 x, f32 y, f32 w, f32 h, f32 minZ, f32 maxZ) {
            if (m_isInitialized && !m_viewportDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setScissor(f32 x, f32 y, f32 w, f32 h) {
            if (m_isInitialized && !m_scissorDynamic) {
                return;
            }
            m_scissorIsSet = true;
        ...
        void GraphicsPipeline::addDynamicState(VkDynamicState state) {
            bool exists = m_dynamicState.some([state](VkDynamicState s) {
                return s == state;
            });
            if (exists) {
                return;
        ...
        void GraphicsPipeline::setPrimitiveType(PRIMITIVE_TYPE ptype) {
            if (m_isInitialized && !m_primTypeDynamic) {
                return;
            }
            m_primType = ptype;
        }
        ...
        void GraphicsPipeline::setPrimitiveRestart(bool enabled) {
            if (m_isInitialized && !m_primRestartEnabledDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setPolygonMode(POLYGON_MODE pmode) {
            if (m_isInitialized && !m_polyModeDynamic) {
                return;
            }
            m_polyMode = pmode;
        }
        ...
        void GraphicsPipeline::setDepthClamp(bool enabled) {
            if (m_isInitialized && !m_depthClampEnabledDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setLineWidth(f32 width) {
            if (m_isInitialized && !m_lineWidthDynamic) {
                return;
            }
            m_lineWidth = width;
        }
        ...
        void GraphicsPipeline::setCullMode(CULL_MODE cmode) {
            if (m_isInitialized && !m_cullModeDynamic) {
                return;
            }
            m_cullMode = cmode;
        }
        ...
        void GraphicsPipeline::setFrontFaceMode(FRONT_FACE_MODE ffmode) {
            if (m_isInitialized && !m_frontFaceModeDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setDepthCompareOp(COMPARE_OP op) {
            if (m_isInitialized && !m_depthCompareOpDynamic) {
                return;
            }
            m_depthCompareOp = op;
        }
        ...
        void GraphicsPipeline::setDepthBounds(f32 min, f32 max) {
            if (m_isInitialized && !m_depthBoundsDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setDepthTestEnabled(bool enabled) {
            if (m_isInitialized && !m_depthTestEnabledDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setDepthBoundsTestEnabled(bool enabled) {
            if (m_isInitialized && !m_depthBoundsTestEnabledDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setDepthWriteEnabled(bool enabled) {
            if (m_isInitialized && !m_depthWriteEnabledDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setSrcColorBlendFactor(BLEND_FACTOR factor) {
            if (m_isInitialized && !m_blendEquationDynamic) {
                return;
            }
        ...
        void GraphicsPipeline::setColorBlendEnabled(bool enabled) {
            if (m_isInitialized && !m_colorBlendEnabledDynamic) {
                return;
            }
        ...
        bool GraphicsPipeline::init() {
            ...
            if (m_vertexShader) {
                prog.addShader(m_vertexShader);
            }
            if (m_fragShader) {
                prog.addShader(m_fragShader);
            }
            if (m_geomShader) {
                prog.addShader(m_geomShader);
            }

            if (!prog.link(messageFlags)) {
                error(decomp::String(prog.getInfoLog()));
                return false;
            }

            Array<VkPipelineShaderStageCreateInfo> stages;
            if (m_vertexShader && !processShader(prog, EShLangVertex, stages)) {
                shutdown();
                return false;
            }
            if (m_fragShader && !processShader(prog, EShLangFragment, stages)) {
                shutdown();
                return false;
            }
            if (m_geomShader && !processShader(prog, EShLangGeometry, stages)) {
                shutdown();
                return false;
            }
            ...
            cbai.colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
            ...
            for (u32 i = 0; i < m_storageBuffers.size(); i++) {
                descriptorSetBindings.push({});
                auto& b              = descriptorSetBindings.last();
                auto& s              = m_storageBuffers[i];
                b.binding            = s.binding;
                b.stageFlags         = s.stages;
                b.descriptorCount    = 1;
            ...
            if (vkCreateDescriptorSetLayout(
                    m_device->get(), &dsl, m_device->getInstance()->getAllocator(), &m_descriptorSetLayout
                ) != VK_SUCCESS) {
            ...
            if (m_pushConstantFormat && m_pushConstantFormat->isValid()) {
                pushConstantRanges.push({});
                auto& pc      = pushConstantRanges.last();
                pc.stageFlags = VK_SHADER_STAGE_ALL;
                pc.offset     = 0;
                pc.size       = m_pushConstantFormat->getUniformBlockSize();
            }
            ...
            li.pushConstantRangeCount     = pushConstantRanges.size();
            li.pPushConstantRanges        = pushConstantRanges.size() > 0 ? pushConstantRanges.data() : VK_NULL_HANDLE;

            if (vkCreatePipelineLayout(m_device->get(), &li, m_device->getInstance()->getAllocator(), &m_layout) !=
                VK_SUCCESS) {
            ...
            if (vkCreateGraphicsPipelines(
                    m_device->get(), nullptr, 1, &pci, m_device->getInstance()->getAllocator(), &m_pipeline
                ) != VK_SUCCESS) {
        ...
        void GraphicsPipeline::shutdown() {
            ...
            if (m_layout) {
                vkDestroyPipelineLayout(m_device->get(), m_layout, m_device->getInstance()->getAllocator());
            }
            if (m_descriptorSetLayout) {
                vkDestroyDescriptorSetLayout(
                    m_device->get(), m_descriptorSetLayout, m_device->getInstance()->getAllocator()
                );
            }
            if (m_pipeline) {
                vkDestroyPipeline(m_device->get(), m_pipeline, m_device->getInstance()->getAllocator());
            }
        ...
        bool GraphicsPipeline::recreate() {
            ...
            if (m_vertexShaderSrc.size() > 0 && !setVertexShader(m_vertexShaderSrc)) {
                return false;
            }
            if (m_fragShaderSrc.size() > 0 && !setFragmentShader(m_fragShaderSrc)) {
                return false;
            }
            if (m_geomShaderSrc.size() > 0 && !setGeometryShader(m_geomShaderSrc)) {
                return false;
            }
        ...
        bool GraphicsPipeline::processShader(
            glslang::TProgram& prog, EShLanguage lang, Array<VkPipelineShaderStageCreateInfo>& stages
        ) {
            ...
            decomp::String allLogs = logger.getAllMessages();
            auto msgs              = allLogs.split("\n");
            for (u32 i = 0; i < msgs.size(); i++) {
                decomp::String& ln = msgs[i];
                ...
                if (idx > 0) {
                    ...
                    warn(decomp::String(&ln[len], ln.size() - len));
                    continue;
                }
                ...
                if (idx > 0) {
                    ...
                    error(decomp::String(&ln[len], ln.size() - len));
                    continue;
                }
            }
            ...
            VkResult result = vkCreateShaderModule(m_device->get(), &ci, m_device->getInstance()->getAllocator(), &mod);
            ...
            switch (lang) {
                ...
                case EShLangTessControl: {
                    si.stage = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
                    break;
                }
                case EShLangTessEvaluation: {
                    si.stage = VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
                    break;
                }
                case EShLangGeometry: {
                    si.stage = VK_SHADER_STAGE_GEOMETRY_BIT;
                    break;
                }
                ...
                case EShLangRayGen: {
                    si.stage = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
                    break;
                }
                case EShLangIntersect: {
                    si.stage = VK_SHADER_STAGE_INTERSECTION_BIT_KHR;
                    break;
                }
                case EShLangAnyHit: {
                    si.stage = VK_SHADER_STAGE_ANY_HIT_BIT_KHR;
                    break;
                }
                case EShLangClosestHit: {
                    si.stage = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
                    break;
                }
                case EShLangMiss: {
                    si.stage = VK_SHADER_STAGE_MISS_BIT_KHR;
                    break;
                }
                case EShLangCallable: {
                    si.stage = VK_SHADER_STAGE_CALLABLE_BIT_KHR;
                    break;
                }
                case EShLangTask: {
                    si.stage = VK_SHADER_STAGE_TASK_BIT_EXT;
                    break;
                }
                case EShLangMesh: {
                    si.stage = VK_SHADER_STAGE_MESH_BIT_EXT;
                    break;
                }
            }
