...
namespace render {
    namespace vulkan {
        ...
        const String& PhysicalDevice::getName() const {
            return m_name;
        }
        ...
        bool PhysicalDevice::isExtensionAvailable(const String& name) const {
        ...
        bool PhysicalDevice::isLayerAvailable(const String& name) const {
        ...
        bool PhysicalDevice::canPresentToSurface(Surface* surface, const QueueFamily& queueFamily) const {
            ...
            if (vkGetPhysicalDeviceSurfaceSupportKHR(m_handle, queueFamily.getIndex(), surface->get(), &canPresent) !=
                VK_SUCCESS) {
                return false;
            }
        ...
        bool PhysicalDevice::getSurfaceSwapChainSupport(Surface* surface, SwapChainSupport* out) const {
            if (!surface || !out || !m_handle) {
                return false;
            }

            if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_handle, surface->get(), &out->m_capabilities) !=
                VK_SUCCESS) {
                return false;
            }
            ...
            if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_handle, surface->get(), &count, out->m_formats.data()) !=
                VK_SUCCESS) {
                out->m_capabilities = {};
                out->m_formats.clear();
                return false;
            ...
            if (vkGetPhysicalDeviceSurfacePresentModesKHR(
                    m_handle, surface->get(), &count, out->m_presentModes.data()
                ) != VK_SUCCESS) {
                out->m_capabilities = {};
                out->m_formats.clear();
        ...
        bool PhysicalDevice::getMemoryTypeIndex(const VkMemoryRequirements& reqs, VkMemoryPropertyFlags flags, u32* dst)
            const {
            if (!dst) {
                return false;
            }
        ...
        Array<PhysicalDevice> PhysicalDevice::list(Instance* instance) {
            ...
            for (u32 i = 0; i < count; i++) {
                ...
                dev.m_name = dev.m_props.deviceName;
                ...
                if (vkEnumerateDeviceExtensionProperties(
                        devices[i], nullptr, &extCount, dev.m_availableExtensions.data()
                    ) != VK_SUCCESS) {
                    out.clear();
                    delete[] devices;
                    return {};
                ...
                if (vkEnumerateDeviceLayerProperties(devices[i], &layerCount, dev.m_availableLayers.data()) !=
                    VK_SUCCESS) {
                    out.clear();
                    delete[] devices;
                    return {};
