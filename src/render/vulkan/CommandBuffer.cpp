...
namespace render {
    namespace vulkan {
        ...
        void CommandBuffer::bindVertexBuffer(VertexBuffer* vbo, u32 off) {
            if (!m_buffer || !m_isRecording) {
                return;
            }
            VkDeviceSize offset = off;
            VkBuffer buf        = vbo->getBuffer();
            vkCmdBindVertexBuffers(m_buffer, 0, 1, &buf, &offset);
        }

        void CommandBuffer::bindVertexBuffer(Buffer* vbo, u32 off) {
            if (!m_buffer || !m_isRecording) {
                return;
            }
            VkDeviceSize offset = off;
        ...
        void CommandBuffer::updatePushConstants(u32 offset, u32 size, const void* data, VkShaderStageFlags stages) {
            if (!m_buffer || !m_isRecording || !m_boundPipeline) {
                return;
            }
            vkCmdPushConstants(m_buffer, m_boundPipeline->getLayout(), stages, offset, size, data);
        }
        ...
        void CommandBuffer::draw(Vertices* vertices) {
            ...
            vkCmdDraw(m_buffer, vertices->getCount(), 1, 0, 0);
        }
    };
};