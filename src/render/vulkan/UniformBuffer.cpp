...
namespace render {
    namespace vulkan {
        ...
        UniformBuffer::UniformBuffer(LogicalDevice* device, core::DataFormat* fmt, u32 objectCapacity)
            : m_buffer(device), m_stagingBuffer(device) {
            m_device           = device;
            m_fmt              = fmt;
            ...
            if (alignment > 0) {
                m_paddedObjectSize = (m_paddedObjectSize + alignment - 1) & ~(alignment - 1);
            }
        ...
        void UniformBuffer::free(UniformObject* n) {
            if (!m_buffer.isValid() || !m_used || !n || n->m_buffer != this) {
                return;
            }

            if (n->m_last) {
                n->m_last->m_next = n->m_next;
            }
            if (n->m_next) {
                n->m_next->m_last = n->m_last;
            }
        ...
        void UniformBuffer::submitUpdates(CommandBuffer* cb) {
            if (!m_buffer.isValid() || !m_hasUpdates) {
                return;
            }
            ...
            vkCmdCopyBuffer(cb->get(), m_stagingBuffer.get(), m_buffer.get(), m_copyRanges.size(), m_copyRanges.data());
        ...
        void UniformBuffer::updateObject(UniformObject* n, const void* data) {
            ...
            m_hasUpdates      = true;
            m_objUpdated[idx] = 1;
            m_minUpdateIdx    = ::decomp::min(m_minUpdateIdx, idx);
            m_maxUpdateIdx    = ::decomp::max(m_maxUpdateIdx, idx);
        }
        ...
        void UniformObject::write(const void* data) {
            m_buffer->updateObject(this, data);
        }

        //
        // UniformObjectFactory
        //

        UniformObjectFactory::UniformObjectFactory(LogicalDevice* device, u32 maxObjectsPerBuffer) {
            m_device              = device;
            m_maxObjectsPerBuffer = maxObjectsPerBuffer;
        }

        UniformObjectFactory::~UniformObjectFactory() {
            freeAll();
        }

        void UniformObjectFactory::freeAll() {
            m_formats.clear();
        ...
        UniformObject* UniformObjectFactory::allocate(core::DataFormat* fmt) {
