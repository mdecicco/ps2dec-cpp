...
namespace render {
    namespace vulkan {
        ...
        bool LogicalDevice::enableExtension(const String& name) {
            if (m_isInitialized || !m_physicalDevice) {
                return false;
            }
            ...
            if (!m_physicalDevice->isExtensionAvailable(name)) {
                return false;
            }
        ...
        bool LogicalDevice::isExtensionEnabled(const String& name) const {
        ...
        bool LogicalDevice::enableLayer(const String& name) {
            if (m_isInitialized || !m_physicalDevice) {
                return false;
            }
            ...
            if (!m_physicalDevice->isLayerAvailable(name)) {
                return false;
            }
        ...
        bool LogicalDevice::isLayerEnabled(const String& name) const {
        ...
        bool LogicalDevice::init(bool needsGraphics, bool needsCompute, bool needsTransfer, Surface* surface) {
            ...
            di.enabledExtensionCount   = enabledExtensions.size();
            di.ppEnabledExtensionNames = enabledExtensions.data();
            di.enabledLayerCount       = enabledLayers.size();
            di.ppEnabledLayerNames     = enabledLayers.data();
        ...
        u32 LogicalDevice::buildQueueInfo(
            ...
            for (u32 i = 0; i < families.size(); i++) {
                if (needsGraphics && gfxIdx == -1 && families[i].supportsGraphics()) {
                    gfxIdx = i;
                    if (needsCompute && cmpIdx != i && families[i].supportsCompute()) {
                        cmpIdx = i;
                    }
                    if (needsTransfer && xfrIdx != i && families[i].supportsTransfer()) {
                        xfrIdx = i;
                    }
                    if (surface && srfIdx != i && m_physicalDevice->canPresentToSurface(surface, families[i])) {
                        srfIdx = i;
                    }
                }
                ...
                if (needsCompute && cmpIdx == -1 && families[i].supportsCompute()) {
                    cmpIdx = i;
                    if (needsGraphics && gfxIdx != i && families[i].supportsGraphics()) {
                        gfxIdx = i;
                    }
                    if (needsTransfer && xfrIdx != i && families[i].supportsTransfer()) {
                        xfrIdx = i;
                    }
                    if (surface && srfIdx != i && srfIdx != gfxIdx &&
                        m_physicalDevice->canPresentToSurface(surface, families[i])) {
                        srfIdx = i;
                    }
                }
                ...
                if (gfxSatisfied && cmpSatisfied && xfrSatisfied && srfSatisfied) {
                    break;
                }
            }
