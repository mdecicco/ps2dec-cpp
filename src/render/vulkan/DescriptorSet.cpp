...
namespace render {
    namespace vulkan {
        ...
        bool DescriptorPool::init() {
            ...
            poolSizes[0].descriptorCount      = m_maxSets;
            poolSizes[1].type                 = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            poolSizes[1].descriptorCount      = m_maxSets;
            poolSizes[2].type                 = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            poolSizes[2].descriptorCount      = m_maxSets;
            ...
            if (vkCreateDescriptorPool(m_device->get(), &pi, m_device->getInstance()->getAllocator(), &m_pool) !=
                VK_SUCCESS) {
        ...
        void DescriptorPool::resetNodes() {
            for (u32 i = 0; i < m_maxSets; i++) {
                ...
                if (i == 0) {
                    m_sets[i].m_last = nullptr;
                } else {
                    m_sets[i].m_last = &m_sets[i - 1];
                }

                if (i == m_maxSets - 1) {
                    m_sets[i].m_next = nullptr;
                } else {
                    m_sets[i].m_next = &m_sets[i + 1];
                }
            }
        ...
        void DescriptorSet::add(Texture* tex, u32 bindingIndex) {
            for (u32 i = 0; i < m_descriptors.size(); i++) {
                if (m_descriptors[i].bindingIdx == bindingIndex) {
                    m_descriptors[i].uniform       = nullptr;
                    m_descriptors[i].tex           = tex;
                    m_descriptors[i].storageBuffer = nullptr;
                    return;
                }
            }

            m_descriptors.push({nullptr, tex, nullptr, bindingIndex});
        }

        void DescriptorSet::add(UniformObject* uo, u32 bindingIndex) {
            for (u32 i = 0; i < m_descriptors.size(); i++) {
                if (m_descriptors[i].bindingIdx == bindingIndex) {
                    m_descriptors[i].uniform       = uo;
                    m_descriptors[i].tex           = nullptr;
                    m_descriptors[i].storageBuffer = nullptr;
                    return;
                }
            }

            m_descriptors.push({uo, nullptr, nullptr, bindingIndex});
        }
        ...
        void DescriptorSet::add(Buffer* storageBuffer, u32 bindingIndex) {
            for (u32 i = 0; i < m_descriptors.size(); i++) {
                if (m_descriptors[i].bindingIdx == bindingIndex) {
                    m_descriptors[i].uniform       = nullptr;
                    m_descriptors[i].tex           = nullptr;
                    m_descriptors[i].storageBuffer = storageBuffer;
                    return;
                }
            }

            m_descriptors.push({nullptr, nullptr, storageBuffer, bindingIndex});
        }
        ...
        DescriptorSet* DescriptorFactory::allocate(Pipeline* pipeline) {
            ...
            if (found) {
                return found->allocate(pipeline);
            }
