...
namespace render {
    namespace vulkan {
        ...
        ShaderCompiler::ShaderCompiler(LogicalDevice* device) : decomp::IWithLogging("Shader Compiler") {
            m_device        = device;
        ...
        glslang::TShader* ShaderCompiler::compileShader(const String& source, EShLanguage type) {
            ...
            bool preprocessSuccess = shdr->preprocess(
                &resources, 450, ENoProfile, false, false, messageFlags, &preprocessOutput, noIncludes
            );

            if (!preprocessSuccess) {
                error(decomp::String(shdr->getInfoLog()));
                delete shdr;
                return nullptr;
            }
