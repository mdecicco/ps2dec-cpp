...
namespace render {
    namespace vulkan {
        ...
        Framebuffer::attachment& Framebuffer::attach(VkImageView view, VkFormat format) {
            m_attachments.push({view, format, {0, 0, 0, 0}});

            return m_attachments.last();
        }

        Framebuffer::attachment& Framebuffer::attach(Texture* texture) {
            m_attachments.push({texture->getView(), texture->getFormat(), {0, 0, 0, 0}});

            return m_attachments.last();
        }

        bool Framebuffer::init(const vec2ui& dimensions) {
            if (m_framebuffer || m_attachments.size() == 0) {
                return false;
            }
            ...
            if (vkCreateFramebuffer(dev->get(), &fbi, dev->getInstance()->getAllocator(), &m_framebuffer) !=
                VK_SUCCESS) {
                dev->getInstance()->error(decomp::String::Format("Failed to create framebuffer"));
                shutdown();
                return false;
            }
