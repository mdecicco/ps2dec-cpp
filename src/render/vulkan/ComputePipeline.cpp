...
namespace render {
    namespace vulkan {
        ComputePipeline::ComputePipeline(ShaderCompiler* compiler, LogicalDevice* device)
            : Pipeline(device), decomp::IWithLogging("Compute Pipeline") {
            m_device              = device;
            m_compiler            = compiler;
        ...
        bool ComputePipeline::init() {
            if (m_pipeline || !m_device || !m_compiler) {
                return false;
            }
            ...
            if (m_computeShader) {
                prog.addShader(m_computeShader);
            }
            ...
            if (m_computeShader && !processShader(prog, EShLangCompute, stages)) {
                shutdown();
                return false;
            }
            ...
            if (vkCreateComputePipelines(
                    m_device->get(), VK_NULL_HANDLE, 1, &pi, m_device->getInstance()->getAllocator(), &m_pipeline
                ) != VK_SUCCESS) {
        ...
        bool ComputePipeline::recreate() {
            ...
            if (m_computeShaderSrc.size() > 0 && !setComputeShader(m_computeShaderSrc)) {
                return false;
            }
