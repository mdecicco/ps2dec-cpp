...
namespace render {
    namespace utils {
        ...
        bool SimpleDebugDraw::init(
            ...
            vulkan::UniformObjectFactory* uboFactory,
            ...
            m_projection = mat4f::Perspective(::decomp::radians(70.0f), f32(e.width) / f32(e.height), 0.1f, 100.0f);
            m_view       = mat4f::LookAt(vec3f(10.0f, 10.0f, 10.0f), vec3f(0.0f, 0.0f, 0.0f), vec3f(0.0f, 1.0f, 0.0f));
            m_manualProjection = false;
            m_manualView       = false;
            for (u32 i = 0; i < 256; i++) {
                m_keyDown[i] = false;
        ...
        void SimpleDebugDraw::shutdown() {
            m_frameVertices.each([](vulkan::Vertices* v) {
                v->free();
            });
            m_frameDescriptorSets.each([](vulkan::DescriptorSet* s) {
                s->free();
            });
            m_frameUniforms.each([](vulkan::UniformObject* u) {
                u->free();
            });
        ...
        vulkan::GraphicsPipeline* SimpleDebugDraw::getPipeline() const {
            return m_pipeline;
        }
        ...
        void SimpleDebugDraw::begin(u32 currentSwapChainImageIndex) {
            ...
            if (!m_manualView) {
                if (m_keyDown[u32(::decomp::KeyboardKey::W)]) {
                    m_moveVelocity.z += m_moveAccel * m_moveSpeed * dt;
                }
                if (m_keyDown[u32(::decomp::KeyboardKey::S)]) {
                    m_moveVelocity.z -= m_moveAccel * m_moveSpeed * dt;
                }
                if (m_keyDown[u32(::decomp::KeyboardKey::A)]) {
                    m_moveVelocity.x += m_moveAccel * m_moveSpeed * dt;
                }
                if (m_keyDown[u32(::decomp::KeyboardKey::D)]) {
                    m_moveVelocity.x -= m_moveAccel * m_moveSpeed * dt;
                }

                m_moveVelocity.x = ::decomp::clamp(m_moveVelocity.x, -m_moveSpeed, m_moveSpeed);
                m_moveVelocity.y = ::decomp::clamp(m_moveVelocity.y, -m_moveSpeed, m_moveSpeed);
                m_moveVelocity.z = ::decomp::clamp(m_moveVelocity.z, -m_moveSpeed, m_moveSpeed);
        ...
        void SimpleDebugDraw::onMouseMove(i32 x, i32 y) {
            ...
            if (m_btnDown && !m_manualView) {
                ...
                vec2f delta = {cur.x - m_cursor.x, cur.y - m_cursor.y};

                vec2f scale = {fabsf(delta.x) / f32(e.width), fabsf(delta.y) / f32(e.height)};

                vec3f xAxis = vec3f(1.0f, 0.0f, 0.0f);
                vec3f yAxis = m_view.basis() * vec3f(0.0f, 1.0f, 0.0f);

                if (delta.x < 0.0f) {
                    m_view *= mat4f::Rotation(yAxis, ::decomp::radians(rotSpeed * scale.x));
                } else if (delta.x > 0.0f) {
                    m_view *= mat4f::Rotation(yAxis, ::decomp::radians(-rotSpeed * scale.x));
                }

                if (delta.y < 0.0f) {
                    m_view *= mat4f::Rotation(xAxis, ::decomp::radians(rotSpeed * scale.y));
        ...
        void SimpleDebugDraw::onScroll(f32 delta) {
            ...
            m_moveSpeed += delta;
            m_moveSpeed = ::decomp::clamp(m_moveSpeed, 0.1f, 100.0f);
        }

        void SimpleDebugDraw::onMouseDown(::decomp::MouseButton buttonIdx) {
            if (buttonIdx != ::decomp::MouseButton::Left) {
                return;
            }
            m_btnDown = true;
        }

        void SimpleDebugDraw::onMouseUp(::decomp::MouseButton buttonIdx) {
            if (buttonIdx != ::decomp::MouseButton::Left) {
                return;
            }
            m_btnDown = false;
        ...
        void SimpleDebugDraw::onKeyDown(::decomp::KeyboardKey key) {
            m_keyDown[u32(key)] = true;
        }

        void SimpleDebugDraw::onKeyUp(::decomp::KeyboardKey key) {
            m_keyDown[u32(key)] = false;
        }
    };
};