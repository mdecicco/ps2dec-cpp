...
namespace render {
    IWithRendering::IWithRendering() : decomp::IWithLogging("Render") {
        m_window            = nullptr;
        m_instance          = nullptr;
    ...
    bool IWithRendering::initRendering(::decomp::Window* win) {
        ...
        if (!m_logicalDevice->enableExtension(VK_KHR_SWAPCHAIN_EXTENSION_NAME)) {
            fatal(
                "Selected device '%s' does not support the swapchain extension",
        ...
        m_logicalDevice->getInstance()->addNestedLogger(m_frames);

        m_shaderCompiler = new vulkan::ShaderCompiler(m_logicalDevice);
        addNestedLogger(m_shaderCompiler);
        ...
        m_uboFactory        = new vulkan::UniformObjectFactory(m_logicalDevice, 1024);
        m_descriptorFactory = new vulkan::DescriptorFactory(m_logicalDevice, 256);

        m_window->subscribeInputHandler(this);
    ...
    bool IWithRendering::initDebugDrawing(u32 maxLines) {
        ...
        if (!m_debugDraw->init(
                m_shaderCompiler, m_swapChain, m_renderPass, m_vboFactory, m_uboFactory, m_descriptorFactory, maxLines
            )) {
            delete m_debugDraw;
            return false;
        }

        m_window->subscribeInputHandler(m_debugDraw);
    ...
    void IWithRendering::shutdownRendering() {
        if (m_debugDraw) {
            m_window->unsubscribeInputHandler(m_debugDraw);
        ...
        if (m_window) {
            m_window->unsubscribeInputHandler(this);
            m_window = nullptr;
        }
    ...
    const vulkan::PhysicalDevice* IWithRendering::choosePhysicalDevice(const Array<vulkan::PhysicalDevice>& devices) {
        ...
        for (render::u32 i = 0; i < devices.size(); i++) {
            ...
            if (!devices[i].getSurfaceSwapChainSupport(m_surface, &swapChainSupport)) {
                continue;
            }
            ...
            if (!swapChainSupport.hasFormat(VK_FORMAT_B8G8R8A8_SRGB, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR)) {
                continue;
            }
    ...
    void IWithRendering::onWindowResize(::decomp::Window* win, u32 width, u32 height) {
        if (!m_initialized || win != m_window) {
            return;
        }
    ...
    ::decomp::Window* IWithRendering::getWindow() const {
        return m_window;
    }
