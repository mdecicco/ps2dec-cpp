...
namespace render {
    namespace core {
        FrameContext::FrameContext() : decomp::IWithLogging("Frame") {
            m_swapChain      = nullptr;
            m_buffer         = nullptr;
        ...
        bool FrameContext::begin() {
            if (m_frameStarted || !m_buffer) {
                return false;
            }

            if (vkWaitForFences(m_device->get(), 1, &m_fence, VK_TRUE, UINT64_MAX) != VK_SUCCESS) {
                return false;
            }
            ...
            if (vkAcquireNextImageKHR(
                    m_device->get(), m_swapChain->get(), UINT64_MAX, m_swapChainReady, nullptr, &m_scImageIdx
                ) != VK_SUCCESS) {
                return false;
            }
        ...
        bool FrameContext::end() {
            if (!m_frameStarted || !m_buffer->end()) {
                return false;
            }
        ...
        bool FrameContext::init(vulkan::SwapChain* swapChain, vulkan::CommandBuffer* cb) {
            ...
            if (vkCreateSemaphore(m_device->get(), &si, m_device->getInstance()->getAllocator(), &m_swapChainReady) !=
                VK_SUCCESS) {
            ...
            if (vkCreateSemaphore(m_device->get(), &si, m_device->getInstance()->getAllocator(), &m_renderComplete) !=
                VK_SUCCESS) {
