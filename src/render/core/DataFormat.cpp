...
namespace render {
    namespace core {
        ...
        void DataFormat::addAttr(DATA_TYPE type, u32 offset, u32 elementCount) {
            if (type == dt_struct || elementCount == 0) {
                return;
            }

            u32 sz  = DataFormat::AttributeSize(type) * elementCount;
            u32 usz = DataFormat::AttributeSize(type, true) * elementCount;
            m_attrs.push({type, nullptr, elementCount, offset, sz, usz});

            if (offset + sz > m_size) {
                m_size = offset + sz;
            }

            if (offset + usz > m_uniformBlockSize) {
                m_uniformBlockSize = offset + usz;
            }
        }
        ...
        void DataFormat::addAttr(const DataFormat* type, u32 offset, u32 elementCount) {
            ...
            m_attrs.push({dt_struct, type, elementCount, offset, sz, usz});
        ...
        bool DataFormat::isEqualTo(const DataFormat* rhs) const {
            ...
            if (m_attrs.size() != rhs->m_attrs.size()) {
                return false;
            }

            return !m_attrs.some([this, rhs](const DataFormat::Attribute& tp, u32 idx) {
                auto& attr = rhs->m_attrs[idx];
                if (attr.type != tp.type) {
                    return true;
                }
                if (attr.elementCount != tp.elementCount) {
                    return true;
                }
                if (attr.size != tp.size) {
                    return true;
                }
                if (attr.formatRef != tp.formatRef) {
                    return true;
                }
