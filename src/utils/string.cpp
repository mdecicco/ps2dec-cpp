...
namespace std {
    size_t hash<decomp::String>::operator()(const decomp::String& str) const {
        size_t hash = 5381;

        for (size_t i = 0; i < str.m_len; i++) {
            hash = ((hash << 5) + hash) + str.m_data[i];
        }
...
namespace decomp {
    ...
    String::String(const char* str) : m_data(nullptr), m_len(u32(strlen(str))), m_capacity(m_len), m_readOnly(false) {
        m_data = (char*)new u8[m_capacity + u64(1)];
        memcpy(m_data, str, m_len);
        m_data[m_len] = 0;
    }

    String::String(const std::string& str)
        : m_data(nullptr), m_len(u32(str.size())), m_capacity(m_len), m_readOnly(false) {
        m_data = (char*)new u8[m_capacity + u64(1)];
    ...
    String::String(char* str, u32 len) : m_data(nullptr), m_len(len), m_capacity(len), m_readOnly(false) {
        m_data = (char*)new u8[m_capacity + u32(1)];
    ...
    String& String::operator=(const char* rhs) {
        if (m_readOnly) {
            throw GenericException("Attempted to modify read-only string");
        }
    ...
    void String::replaceAll(const String& str, const String& with) {
        ...
        for (u32 i = 0; i < m_len; i++) {
            if (iidx < indices.size() && i == indices[iidx]) {
                for (u32 j = 0; j < with.m_len; j++) {
                    tmp.m_data[tmp.m_len++] = with.m_data[j];
                }
                i += (slen - 1);
                iidx++;
                continue;
    ...
    String String::trim() const {
        ...
        for (u32 i = 0; i < m_len; i++) {
            ...
            if (isspace(m_data[i])) {
                ...
                if (bidx) {
                    for (u32 b = 0; b < bidx; b++) {
                        out.m_data[out.m_len++] = buf[b];
                    }
                    bidx = 0;
                }
    ...
    String String::toUpperCase() const {
        String out(*this);
        for (u32 i = 0; i < m_len; i++) {
            out.m_data[i] = toupper(out.m_data[i]);
        }
        return out;
    }
    ...
    String String::toLowerCase() const {
        String out(*this);
        for (u32 i = 0; i < m_len; i++) {
            out.m_data[i] = tolower(out.m_data[i]);
        }
        return out;
    }

    Array<String> String::split(std::initializer_list<const char*> delimiters, const char* notBetween, u32 maxCount)
        const {
        Array<String> out;
        if (m_len == 0) {
            return out;
        ...
        for (u32 i = 0; i < m_len; i++) {
            ...
            if (!hasMax || maxCount > 0) {
                for (const char* d : delimiters) {
                    ...
                    u32 j = 0;
                    for (; j < slen && match; j++) {
                        match = m_data[i + j] == d[j];
                    }
                    if (match) {
                        matchedAny = true;
                        if ((i + slen) > maxMatchEndPos) {
                            maxMatchEndPos = i + slen;
                        }
                    }
                }
            }
    ...
    Array<u32> String::indicesOf(const String& str) const {
        Array<u32> out;
        if (str.m_len == 0 || str.m_len > m_len) {
            return out;
        }

        for (u32 i = 0; i < m_len; i++) {
            bool match = true;
            for (u32 j = 0; j < str.m_len && match; j++) {
                match = m_data[i + j] == str.m_data[j];
            }
            if (match) {
                out.push(i);
            }
    ...
    void String::resize(u32 cap) {
        ...
        m_capacity = cap;
        char* data = (char*)new u8[cap];
        if (m_len < m_capacity) {
            memset(data + m_len, 0, m_capacity - m_len);
        }
        if (m_len) {
